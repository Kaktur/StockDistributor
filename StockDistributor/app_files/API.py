...
import websocket, json, pytz, time, sys
from datetime import datetime, timedelta

#throttling setting - time to wait between API calls
# in ms (must be less than 1000)
limit = 200
...
class XTB:
    ...
    def get_TradingHours(self, symbol, week = None):
        global oldsymbol
        global data
        if week == None:
            week = datetime.now(pytz.timezone("CET")).strftime('%w')
        ...
        if symbol != oldsymbol:
            ...
            asktime_json = json.dumps(asktime)
            result = self.send(asktime_json)
            result = json.loads(result)
            try:
                tradingtime = result["returnData"][0]["trading"]
            except:
                return result["errorDescr"]
            data = tradingtime
        else:
            tradingtime = data

        time = []
        for i in tradingtime:
            if int(i['day']) == int(week):
                time.append(i)
        
        if time == []:
            return False
        else:
            time=sorted(time,key=lambda x: x["fromT"])
            return time

    #r
    def get_CommissionDef(self, symbol, voliume):
        symbol ={
    ...
    #r
    def get_AccCurency(self):
        UserData ={
            "command": "getCurrentUserData"
        }
        UserData_json = json.dumps(UserData)
        result = self.send(UserData_json)
        result = json.loads(result)
        currency = result["returnData"]["currency"]
        return currency
    ...
    def get_AllSymbols(self):
        ...
        result = json.loads(result)
        try:
            avialblesymbols=result["returnData"]
        except:
            print(result)
            self.logout()
            sys.exit()
        return avialblesymbols
    ...
    def get_ServerTimeStr(self):
            ...
            result = json.loads(result)
            time = result["returnData"]["timeString"]
            return time
    ...
    def make_Trade(self, symbol, cmd, transaction_type, volume, comment="", order=0, sl=0, tp=0, days=0, hours=0, minutes=0):
        ...
        if result["status"]==True:
            ...
            #error
            try:
                return False, result["returnData"]["errorDescr"]
            except:
                return False, 'error'
        """
    ...
    def limitkeeper(self):
        global limit
        global old 
        now = round(time.time()*1000)     
        if ((old + limit) >= now) and (old != 0):
            wait = float("0."+(str((old + limit) - now)))
            print (str(wait))
            time.sleep(wait)
    ...
    def connect(self):
        global real
        if real:
          platform = "wss://ws.xtb.com/real"
        else:
          platform = "wss://ws.xtb.com/demo"
        try:
            self.ws=websocket.create_connection(platform)
            #Success
            return True
        except:
            #Error
